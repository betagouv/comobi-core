...
const MAPBOX_ACCESS_TOKEN =
	'pk.eyJ1IjoiZGF2aWRicnVhbnQiLCJhIjoiY2p5enA0MHNmMDNwbTNsdW9taDA4aWI3dCJ9.3P_tPQT5h2qvjrQYjAQSFQ'
...
export default function CorresplotMap({
	directionsByTrip,
	tripRequest: tripRequestWithStatus,
	...
	const tripRequest = makeTrip(
		tripRequestWithStatus.origin,
		tripRequestWithStatus.destination
	)
	...
	return html`
		...
		>
			...
			${tripRequest && directionsByTrip.has(tripRequest)
				? html`
						<${GeoJSON}
							className="trip-request"
							data=${directionsByTrip.get(tripRequest).geoJSON}
						>
			...
			${[...displayedDriverTrips]
				...
				.map(
					trip => html`
						<${GeoJSON}
							className="driver-trip"
							data=${directionsByTrip.get(trip).geoJSON}
						>
